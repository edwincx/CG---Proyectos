...
void pintarProvincia(vertice* vertices, int numVertices){
	...
	while(scanline > calcularYmin(vertices, numVertices)){
		...
		/*printf("bordes activos\n");
		for (i = 0; i < numVerticesActivos-1; i+=2)
		{
		    printf("(%d,%d),(%d,%d)\n", verticesActivos[i].X,verticesActivos[i].Y,verticesActivos[i+1].X,verticesActivos[i+1].Y);
		}*/

	    int numIntersecciones= calcularIntersecciones(scanline, numVerticesActivos);
	    /*if(numIntersecciones != 0){
	    	printf("numero de vertices Activos:%d\n", numVerticesActivos);
	    	printf("numero de intersecciones:%d\n", numIntersecciones);
		}
	    if(numIntersecciones != 0){
			printf("intersecciones\n");
			for (i = 0; i < numIntersecciones; i++)
			{
			    printf("(%d,%d) \n", intersecciones[i].X,intersecciones[i].Y);
			}
		}*/
		...
		for (i = 0; i < numIntersecciones; i+=2)
		{
			...
			//printf("bresenham2(%d,%d,%d,%d); \n", intersecciones[i].X,intersecciones[i].Y,intersecciones[i+1].X,intersecciones[i+1].Y);
	...
	/*
    setcolor (0.4f, 0.6f, 0.3f);
    bresenham2(intersecciones[0].X,intersecciones[0].Y,intersecciones[1].X,intersecciones[1].Y);
    bresenham2(intersecciones[2].X,intersecciones[2].Y,intersecciones[3].X,intersecciones[3].Y);*/
}
